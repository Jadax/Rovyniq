...
- `apps/api`: bearer-token verification and browser OIDC PKCE/session boundary.
- `docs/`: decisions, security, compliance and operational handover.

## Active milestone

Phase 1 platform direction is selected: self-hosted Keycloak (OIDC), PostgreSQL and SeaweedFS (S3-compatible storage), all with free/open-source routes. Docker Desktop remains required to run the local stack. The next working slice is secure document ingestion for synthetic IRP5 PDFs after a production identity configuration is approved.
