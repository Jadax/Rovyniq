...
6. In ZITADEL's applicable Login Behavior settings, enable local username/password login and self-registration only for the household pilot. Configure the post-registration policy to assign only the `taxpayer` project role after the identity provider's contact-verification step. The application then creates a separate Rovyniq tenant and 2026 ITR12 workspace for that verified subject. Do not grant staff, practitioner or organisation-admin roles through self-registration.
