...
3. Configure a production OIDC provider and same-origin application deployment, then implement the PostgreSQL, encrypted-storage and scanner adapters and authenticated document review UI; replace the synthetic workspace data only after those controls pass.
...
`node --experimental-strip-types --input-type=module -e "…edge API contract…"` — passed (health 200; upload fails closed with 503). `npm.cmd run test:all` — passed: compliance checks plus 18 tests, 0 failures (2026-07-27). Docker was checked and is not installed.
